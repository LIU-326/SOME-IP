...
class SomeipController:
    ...
    def start(self):
        ...
        print("2 - 发送事件订阅请求 (单播到虚拟机)")
        print("3 - 发送订阅确认ACK (单播到虚拟机)")
        print("q - 退出程序")
        print("="*50)
    ...
    def send_offer(self):
        ...
        print(f"[+] 已向多播地址 {EthParameter.sd_ip} 发送服务Offer")
    ...
    def send_subscribe(self):
        ...
        sendp(packet, iface=EthParameter.server_network_card, verbose=True)
        print(f"[+] 已向虚拟机 {EthParameter.client_ip} 发送事件订阅")
    ...
    def send_subscribe_ack(self):
        ...
        sendp(packet, iface=EthParameter.server_network_card, verbose=True)
        print(f"[+] 已向虚拟机 {EthParameter.client_ip} 发送订阅确认ACK")
